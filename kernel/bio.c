...
#include "buf.h"
#define NBUCKET 13
#define NBUF_2 (NBUCKET * 3)

struct {
  struct spinlock lock;
  struct buf buf[NBUF_2];
} bcache;
...
binit(void)
{
  ...
  for(b = bcache.buf; b < bcache.buf+NBUF_2; b++){
    initsleeplock(&b->lock, "buffer");
  }
  ...
  for (int i = 0; i < NBUCKET; i++) {
    ...
    for (int j = 0; j < NBUF_2 / NBUCKET; j++) {
...
brelse(struct buf *b)
{
  ...
  acquire(&hashtable[idx].lock);//使用桶级锁
  b->refcnt--;
  if (b->refcnt == 0) {
    // no one is waiting for it.
    b->timestamp = ticks;
